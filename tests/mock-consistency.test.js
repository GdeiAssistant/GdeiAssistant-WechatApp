...
function makeMockUtils() {
  ...
  return {
    ensureAuthorized: function() { return null },
    readState: function() {
      return { profile: { username: 'gdeiassistant' }, community: communityState }
    },
    writeState: function(state) { communityState = state.community },
    buildSuccess: function(data) { return { success: true, data: data } },
    resolveWithDelay: function(result) { return Promise.resolve(result) },
    cloneValue: function(value) { return JSON.parse(JSON.stringify(value)) }
  }
}
...
test('secondhand mock items have required fields', async function() {
  ...
  items.forEach(function(item) {
    requiresFields(item, ['id', 'name', 'description', 'price', 'state', 'publishTime'], 'secondhand')
  })
})
...
test('lostandfound mock items have required fields', async function() {
  ...
  items.forEach(function(item) {
    requiresFields(item, ['id', 'name', 'description', 'location', 'lostType', 'itemType', 'state', 'publishTime'], 'lostandfound')
  })
})
...
test('express mock items have required fields', async function() {
  ...
  items.forEach(function(item) {
    requiresFields(item, ['id', 'nickname', 'content', 'selfGender', 'personGender', 'publishTime'], 'express')
  })
})
...
test('delivery mock orders have required fields', async function() {
  ...
  orders.forEach(function(order) {
    requiresFields(order, ['orderId', 'name', 'number', 'phone', 'price', 'company', 'address', 'state', 'orderTime'], 'delivery')
  })
})
...
test('mock news data has required fields across all types', function() {
  ...
  typeKeys.forEach(function(type) {
    ...
    assert.ok(Array.isArray(items) && items.length > 0, 'news type ' + type + ' should not be empty')
...
test('endpoints.js returns correct URL patterns for key routes', function() {
  ...
  assert.match(endpoints.info.news(1, 0, 10), /^\/api\/information\/news\/type\/1\/start\/0\/size\/10$/)
  ...
  assert.match(endpoints.community.lostAndFound.lost(0), /^\/api\/lostandfound\/lostitem\/start\/0$/)
  assert.match(endpoints.community.express.list(0, 10), /^\/api\/express\/start\/0\/size\/10$/)
  assert.match(endpoints.community.delivery.list(0, 10), /^\/api\/delivery\/order\/start\/0\/size\/10$/)
  ...
  assert.match(endpoints.community.photograph.list(1, 0, 10), /^\/api\/photograph\/type\/1\/start\/0\/size\/10$/)
  assert.match(endpoints.community.dating.list(0, 0), /^\/api\/dating\/profile\/area\/0\/start\/0$/)

  assert.match(endpoints.messages.announcements(0, 5), /^\/api\/information\/announcement\/start\/0\/size\/5$/)
  assert.match(endpoints.messages.interactionList(0, 10), /^\/api\/information\/message\/interaction\/start\/0\/size\/10$/)
})
